...
import useDiscoverSlippageTolerance from '../../hooks/useDiscoverSlippageTolerance'
...
import { Field, replaceDiscoverState, selectCurrency, setRecipient, switchCurrencies, typeInput } from './actions'
import { DiscoverState } from './reducer'

export function useDiscoverState(): AppState['discover'] {
  return useAppSelector((state) => state.discover)
}

export function useDiscoverActionHandlers(): {
...
// from the current discover inputs, compute the best trade and return it.
export function useDerivedDiscoverInfo(toggledVersion: Version | undefined): {
  ...
  const allowedSlippage = useDiscoverSlippageTolerance(bestTrade ?? undefined)
...
export function queryParametersToDiscoverState(parsedQs: ParsedQs): DiscoverState {
...
// updates the discover state to use the defaults for a given network
export function useDefaultsFromURLSearch():
  ...
  useEffect(() => {
    if (!chainId) return
    const parsed = queryParametersToDiscoverState(parsedQs)
